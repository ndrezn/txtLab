...
def main():
	...
	for genre in genres:
		...
		meta['Document'] = meta.apply(lambda row: get_document(row['URL'], genre), axis=1)
		meta.dropna(subset=['Document'])
		print('Documents collected.')
		meta['Reversions'], meta['Human reverts'] = zip(*meta.apply(lambda row: count_reversions(row['Document']), axis=1))
		meta['Edits'] = meta.apply(lambda row: count_edits((row['Document'])), axis=1)
		meta['Average time between edits'], meta['Average time between edits (for edits less than one month apart)'] = zip(*meta.apply(lambda row: average_time((row['Document'])), axis=1))
		meta['Minor edits'] = meta.apply(lambda row: total_minor((row['Document'])), axis=1)
		meta['Time of first edit'] = meta.apply(lambda row: first_time((row['Document'])), axis=1)
		meta['Length of current version'] = meta.apply(lambda row: article_length((row['Document'])), axis=1)

		users, ratingstimes = zip(*meta.apply(lambda row: count_users_and_ratings((row['Document'])), axis=1))
		meta['Ratings'], meta['Time between rating changes'] = zip(*ratingstimes)
		meta['Users'] = users
		meta['Added words'], meta['Removed words'] = zip(*meta.apply(lambda row: get_added_words((row['Document'])), axis=1))
