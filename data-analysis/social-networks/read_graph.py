...
def genre_purity(communities, graphs):
    ...
    for louvain, g in zip(communities, graphs):
        for group in louvain:
            ...
            total_nodes = 0
            for node in group:
                if ("comedy" == g.vs["genre"][node].lower()) or (
                    "drama" == g.vs["genre"][node].lower()
                ):
                    total_nodes += 1
...
def main():
    ...
    if "all_mediums_small" in kind or "all_mediums" in kind:
        score, large_groups, total_groups, max_types = medium_purity(
            communities, graphs
        )
    ...
    (
        ...
        period_max_types,
        purity_by_type,
    ) = period_purity(communities, graphs)
    ...
    if "novels" not in kind:
        genre_score, genre_large_groups, genre_max_types = genre_purity(
            communities, graphs
        )
