...
def meta_as_label(file_list, label):
    ...
    meta = {
        "films": pandas.read_csv(meta_src + "films.csv"),
...
def train_model(
    classifier, feature_vector_train, label, feature_vector_valid, is_neural_net=False
):
...
def test_model():
    ...
    for i in range(0, 10):
        accuracy += train_model(
            naive_bayes.MultinomialNB(), xtrain_count, train_y, xvalid_count
        )
    ...
    for i in range(0, 10):
        accuracy += train_model(
            naive_bayes.MultinomialNB(), xtrain_tfidf, train_y, xvalid_tfidf
        )
    ...
    for i in range(0, 10):
        accuracy += train_model(
            linear_model.LogisticRegression(), xtrain_count, train_y, xvalid_count
        )
    ...
    for i in range(0, 10):
        accuracy += train_model(
            linear_model.LogisticRegression(), xtrain_tfidf, train_y, xvalid_tfidf
        )
