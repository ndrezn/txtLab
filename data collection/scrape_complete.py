...
async def compare_edits(path, title):
	...
	for i in range(0, metadata.__len__(), +sema):
		texts += await asyncio.gather(*(get_text(revid, 0) for revid in revids[i:(i+sema)]))
	...
	for i in range(metadata.__len__()-1, -1, -1):
		...
		change = Change(i, title, time, metadata[i]['revid'], kind[i], users[i],
						comments[i], rating, texts[i], changes(prev, cur),
						changes(cur, prev))
...
def write_change(path, change):
	...
	body = (str(change.revid) + " +++$+++ " + str(change.time) +
			" +++$+++ " + str(change.kind) + " +++$+++ " + str(change.user) +
			" +++$+++ " + str(change.comment)
			+ " +++$+++ " + str(change.rating) + " +++$+++ ")
